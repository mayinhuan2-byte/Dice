(function () {
  ...
  const MATERIALS = [
    { id: "classic", name: "经典银白", short: "经典" },
    { id: "frosted", name: "磨砂银", short: "磨砂" },
    { id: "ice", name: "冰晶蓝", short: "冰晶" },
    { id: "obsidian", name: "黑曜石", short: "曜石" },
    { id: "titanium", name: "钛金属", short: "钛金" },
  ];
  ...
  function updateRollLabels() {
    ...
    el.resultTime.textContent = state.lastRoll ? formatDate(state.lastRoll.time) : "尚未掷骰";
  ...
  function renderDiceVisuals() {
    ...
    visuals.forEach((visual) => {
      ...
      visual.dataset.style = state.settings.material;
      visual.className = `poly-dice dice-${dice.toLowerCase()}${visual.id === "resultDiceVisual" ? " result-dice" : ""}`;
      visual.dataset.dice = dice;
      visual.dataset.style = state.settings.material;
    });
  }

  function createDiceMarkup(dice, material) {
    return `
      <div class="poly-dice dice-${dice.toLowerCase()}" data-dice="${dice}" data-style="${material}">
        <span class="face face-top">2</span>
        <span class="face face-left">1</span>
        <span class="face face-front">3</span>
        <span class="face face-right">4</span>
        <span class="face face-small-a">2</span>
        <span class="face face-small-b">3</span>
      </div>
    `;
  }
    ...
    el.diceGrid.innerHTML = DICE_TYPES.map((dice) => {
      ...
      return `
        <button class="select-card ${selected ? "selected" : ""}" type="button" data-dice-choice="${dice}" aria-label="选择 ${dice}">
          ...
          <strong>${dice}</strong>
          ${createDiceMarkup(dice, state.settings.material)}
          <span>结果：1-4</span>
        </button>
      `;
    }).join("");
    ...
    el.styleGrid.innerHTML = MATERIALS.map((material) => {
      ...
      return `
        <button class="style-card ${selected ? "selected" : ""}" type="button" data-style-choice="${material.id}" aria-label="${material.name}">
          ${selected ? '<span class="checkmark">✓</span>' : ""}
          ${createDiceMarkup("D20", material.id)}
  ...
  function renderHistory() {
    ...
    el.historyList.innerHTML = items.map((item) => `
      <article class="history-item">
        <div class="mini-dice">${item.result}</div>
  ...
  function saveLastRoll() {
    if (!state.lastRoll) {
      showToast("请先掷骰");
      return;
    }
    if (state.savedLastRollId === state.lastRoll.id) {
      showToast("这次结果已保存");
      return;
    }
  ...
  function bindEvents() {
    ...
    $("#saveButton").addEventListener("click", saveLastRoll);
